...
} from "../../src/test-mode/mock-ticketing-service";
...
describe("MockTicketingService", () => {
  ...
  it("matches the venue reference row lengths and outward M extensions", async () => {
    ...
    const rowCounts = (
      predicate: (id: string, section: string) => boolean,
      rows: number,
    ) =>
      Array.from(
        { length: rows },
        (_, index) =>
          seats.filter(
            (seat) =>
              predicate(seat.id, seat.section) &&
              seat.rowLabel === String(index + 1),
          ).length,
      );

    expect(seats).toHaveLength(860);
    for (const section of ["A", "B", "C", "D", "E", "F", "G", "H"]) {
      expect(rowCounts((_id, candidate) => candidate === section, 7)).toEqual([
        10, 10, 10, 10, 10, 10, 10,
      ]);
    }
    expect(rowCounts((_id, section) => section === "L", 4)).toEqual([
      9, 8, 6, 5,
    ]);
    expect(rowCounts((_id, section) => section === "I", 4)).toEqual([
      9, 8, 6, 5,
    ]);
    expect(rowCounts((_id, section) => section === "K", 6)).toEqual([
      10, 10, 10, 10, 10, 10,
    ]);
    expect(rowCounts((_id, section) => section === "J", 6)).toEqual([
      10, 10, 10, 10, 10, 10,
    ]);
    expect(rowCounts((id) => id.startsWith("seat-M-left-"), 6)).toEqual([
      10, 10, 10, 10, 10, 12,
    ]);
    expect(rowCounts((id) => id.startsWith("seat-M-right-"), 6)).toEqual([
      10, 10, 10, 10, 10, 12,
    ]);

    const xRange = (idPrefix: string, row: string) => {
      const xValues = seats
        .filter((seat) => seat.id.startsWith(idPrefix) && seat.rowLabel === row)
        .map((seat) => seat.x);
  ...
  it("atomically rejects a second reservation for an allocated seat", async () => {
    ...
    await expect(
      service.reserveSeats(eventId, [seat.id]),
  ...
  it("releases all held seats when an order expires", async () => {
    ...
    const expired = await service.getOrder({
  ...
  it("simulates payment, sold allocations, and one ticket per seat", async () => {
    ...
    await service.updateCustomer(credentials, {
      name: "Ada Lovelace",
    ...
    expect(
      (await service.getSeatMap()).seats
        .filter((seat) => seatIds.includes(seat.id))
  ...
  it("restores the deterministic sample after reset", async () => {
    ...
    await service.reset();
    expect((await service.checkinTicket("TUM-DEMO-001")).result).toBe(
      "CHECKED_IN",
    );
  });
});
