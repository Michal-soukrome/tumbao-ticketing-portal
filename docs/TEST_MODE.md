...
- production startup fails fast if the flag is true;
- the Supabase module is dynamically imported only in production mode, so creating test mode does not instantiate a client;
- the mock module contains no network calls and persists only under the namespaced `tumbao:test-data:v1` local-storage key;
...
Reset from the yellow test-mode banner or clear the `tumbao:test-data:v1` local-storage key. Resetting never touches Supabase.
