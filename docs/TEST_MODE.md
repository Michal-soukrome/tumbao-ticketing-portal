...
- the mock module contains no network calls and persists only under the namespaced `tumbao:test-data:v4` local-storage key;
...
Reset from the yellow test-mode banner or clear the `tumbao:test-data:v4` local-storage key. Resetting never touches Supabase.
