...
function requiredEnv(name: "VITE_SUPABASE_URL" | "VITE_SUPABASE_ANON_KEY") {
  ...
  if (!value)
    throw new Error(`${name} is required when VITE_TEST_MODE is not true.`);
  return value;
}
...
class SupabaseTicketingService implements TicketingService {
  ...
  getSeatMap: TicketingService["getSeatMap"] = () =>
    this.invoke("get-seat-map");
  reserveSeats: TicketingService["reserveSeats"] = (eventId, seatIds) =>
    this.invoke("reserve-seats", { event_id: eventId, seat_ids: seatIds });
  updateCustomer: TicketingService["updateCustomer"] = (
    credentials,
    customer,
  ) =>
    this.invoke("update-order-customer", {
      order_id: credentials.orderId,
      access_token: credentials.accessToken,
      customer,
    });
  getOrder: TicketingService["getOrder"] = (credentials) =>
    this.invoke("get-order-status", {
      order_id: credentials.orderId,
      access_token: credentials.accessToken,
    });
  cancelOrder: TicketingService["cancelOrder"] = (credentials) =>
    this.invoke("cancel-order", {
      order_id: credentials.orderId,
      access_token: credentials.accessToken,
    });
  startPayment: TicketingService["startPayment"] = (credentials) =>
    this.invoke("create-payment", {
      order_id: credentials.orderId,
      access_token: credentials.accessToken,
    });
  getAdminStats: TicketingService["getAdminStats"] = () =>
    this.invoke("get-admin-stats");
