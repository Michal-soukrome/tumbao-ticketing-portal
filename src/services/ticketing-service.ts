...
export interface TicketingService {
  ...
  updateCustomer(credentials: OrderCredentials, customer: CustomerDetails): Promise<OrderDto>
  getOrder(credentials: OrderCredentials): Promise<OrderDto>
  startPayment(credentials: OrderCredentials): Promise<{ kind: 'redirect'; url: string } | { kind: 'paid'; order: OrderDto }>
