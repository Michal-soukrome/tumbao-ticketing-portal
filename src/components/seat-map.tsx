...
function VenueFixtures() {
  const sectionLabels = [
    ['D', 255, 39], ['C', 389, 39], ['B', 509, 39], ['A', 640, 39],
    ['H', 255, 149], ['G', 389, 149], ['F', 509, 149], ['E', 640, 149],
    ['L', 260, 259], ['K', 389, 259], ['J', 509, 259], ['I', 634, 259],
  ] as const

  return <g className="venue-fixtures" data-testid="interactive-venue-plan" aria-hidden="true">
    <rect className="venue-floor" x="1" y="1" width="898" height="398" rx="12" />
    <rect className="venue-stage" x="311" y="5" width="274" height="22" rx="1" />
    <text className="venue-stage-label" x="448" y="19">STAGE</text>
    {sectionLabels.map(([label, x, y]) => <text key={label} className="venue-section-label" x={x} y={y}>SECTION {label}</text>)}
    <text className="venue-section-label" x="112" y="108" transform="rotate(17 112 108)">SECTION M</text>
    <text className="venue-section-label" x="800" y="108" transform="rotate(-17 800 108)">SECTION M</text>
    <g className="venue-stairs" transform="translate(176 197)">
      <circle r="22" />
      {[-75, -45, -15, 15, 45, 75].map((angle) => <line key={angle} x1="0" y1="0" x2="22" y2="0" transform={`rotate(${angle})`} />)}
    </g>
    <g className="venue-stairs" transform="translate(722 197)">
      <circle r="22" />
      {[-105, -135, -165, 105, 135, 165].map((angle) => <line key={angle} x1="0" y1="0" x2="22" y2="0" transform={`rotate(${angle})`} />)}
    </g>
    <text className="venue-entrance-label" x="450" y="374">ENTRANCE</text>
  </g>
}
...
export function SeatMap({ seats, selected, ownedHeld = new Set(), selectionLocked = false, onToggle }: SeatMapProps) {
  ...
  const isTap = (event: ReactMouseEvent<SVGRectElement>) => {
    ...
    return !pointerStart || Math.hypot(event.clientX - pointerStart.x, event.clientY - pointerStart.y) <= 4
  }
  ...
  return <div className="seat-map-card">
    ...
    <div className="seat-map-viewport">
      <svg
        ...
        onPointerDownCapture={(event) => { pointerStartRef.current = { x: event.clientX, y: event.clientY } }}
      >
        <VenueFixtures />
        {seats.map((seat) => {
          ...
          return <g key={seat.id} transform={`translate(${seat.x} ${seat.y}) rotate(${seat.rotation})`}>
            ...
            />
            <text className="seat-number" x="0" y="1.6" aria-hidden="true">{seat.seatNumber}</text>
          </g>
        })}
      </svg>
    </div>
