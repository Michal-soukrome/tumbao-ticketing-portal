...
function createInitialState(now: () => number): MockState {
  ...
  for (const seatId of sample.seatIds) allocations[seatId] = { orderId: sample.id, status: 'SOLD' }
  ...
  for (const seatId of seed.initialHeldSeatIds) {
    allocations[seatId] = { orderId: heldOrderId, status: 'HELD', holdExpiresAt: heldExpiry }
  }
  ...
  const orders: StoredOrder[] = [
    ...
    },
    {
      ...
      status: 'PENDING',
      totalMinor: seats.find((seat) => seat.id === seed.initialHeldSeatIds[0])?.priceMinor ?? 0,
      currency: event.currency,
  ...
  return { version: 1, event, seats, allocations, orders, tickets, sequence: 3, admins: seed.admins as MockState['admins'] }
}
...
export class MockTicketingService implements TicketingService {
  ...
  private sweepExpired(state: MockState) {
    ...
    for (const order of state.orders) {
      ...
      for (const seatId of order.seatIds) {
        ...
        if (allocation?.orderId === order.id && allocation.status === 'HELD') delete state.allocations[seatId]
      }
      changed = true
    }
    return changed
  ...
  private requireOrder(state: MockState, credentials: OrderCredentials): StoredOrder {
    const order = state.orders.find(
      (candidate) => candidate.id === credentials.orderId && candidate.accessToken === credentials.accessToken,
    )
    if (!order) throw new AppError('ORDER_NOT_FOUND', 'The order could not be found.')
    return order
  }
  ...
  private toOrder(state: MockState, order: StoredOrder): OrderDto {
    ...
    return {
      ...
      tickets: state.tickets.filter((ticket) => ticket.id.startsWith(`ticket-${order.id}-`) || order.id === seed.samplePaidOrder.id && ticket.id.startsWith('ticket-sample-')),
    }
  }

  async getSeatMap() {
    const state = this.readCurrent()
    return { event: state.event, seats: this.seatsFor(state, state.seats.map((seat) => seat.id)), serverTime: new Date(this.now()).toISOString() }
  }

  async reserveSeats(eventId: string, seatIds: string[]): Promise<ReservationDto> {
    const state = this.readCurrent()
    if (eventId !== state.event.id) throw new AppError('VALIDATION_ERROR', 'The event is invalid.')
    const uniqueIds = [...new Set(seatIds)]
    if (uniqueIds.length !== seatIds.length || uniqueIds.length < 1 || uniqueIds.length > 10) {
      throw new AppError('VALIDATION_ERROR', 'Select between 1 and 10 unique seats.')
    }
    const selected = state.seats.filter((seat) => uniqueIds.includes(seat.id))
    if (selected.length !== uniqueIds.length) throw new AppError('VALIDATION_ERROR', 'One or more seats are invalid.')
    const unavailable = uniqueIds.filter((seatId) => state.allocations[seatId])
    if (unavailable.length) throw new AppError('SEAT_UNAVAILABLE', 'One or more seats are no longer available.', unavailable)
    ...
    for (const seatId of uniqueIds) state.allocations[seatId] = { orderId, status: 'HELD', holdExpiresAt: expiresAt }
    this.write(state)
    return { ...this.toOrder(state, order), orderId, accessToken }
  }

  async updateCustomer(credentials: OrderCredentials, customer: CustomerDetails) {
    const state = this.readCurrent()
    const order = this.requireOrder(state, credentials)
    if (order.status !== 'PENDING') throw new AppError('ORDER_NOT_PAYABLE', 'This order can no longer be updated.')
  ...
  async startPayment(credentials: OrderCredentials) {
    const state = this.readCurrent()
    const order = this.requireOrder(state, credentials)
    if (order.status === 'EXPIRED') throw new AppError('ORDER_EXPIRED', 'The reservation has expired.')
    if (order.status !== 'PENDING') throw new AppError('ORDER_NOT_PAYABLE', 'This order is not payable.')
    if (!order.customer?.name || !order.customer.email) throw new AppError('VALIDATION_ERROR', 'Customer details are required.')
    const ownsEverySeat = order.seatIds.every((seatId) => {
      const allocation = state.allocations[seatId]
      return allocation?.orderId === order.id && allocation.status === 'HELD'
    })
    if (!ownsEverySeat) throw new AppError('PAYMENT_RECONCILIATION_REQUIRED', 'The reserved inventory no longer matches this order.')
    ...
    for (const [index, seatId] of order.seatIds.entries()) {
      ...
      state.tickets.push({
        ...
        status: 'VALID',
        seat: { id: seat.id, section: seat.section, rowLabel: seat.rowLabel, seatNumber: seat.seatNumber },
      })
    }
    this.write(state)
  ...
  async getAdminStats(): Promise<AdminStatsDto> {
    ...
    return {
      revenueMinor: state.orders.filter((order) => order.status === 'PAID').reduce((sum, order) => sum + order.totalMinor, 0),
      currency: state.event.currency,
      soldSeats: allocations.filter((allocation) => allocation.status === 'SOLD').length,
      heldSeats: allocations.filter((allocation) => allocation.status === 'HELD').length,
  ...
  async listOrders(): Promise<AdminOrderDto[]> {
    ...
    return [...state.orders].reverse().map((order) => ({
      ...
      seatLabels: this.seatsFor(state, order.seatIds).map((seat) => `${seat.section}-${seat.rowLabel}-${seat.seatNumber}`),
  ...
  async checkinTicket(tokenOrCode: string): Promise<CheckinResult> {
    ...
    const ticket = state.tickets.find((candidate) => candidate.qrToken === normalized || candidate.ticketCode.toLowerCase() === normalized.toLowerCase())
    if (!ticket) return { result: 'INVALID' }
    if (ticket.status === 'VOID') return { result: 'VOID', ticket }
    if (ticket.status === 'CHECKED_IN') return { result: 'ALREADY_USED', ticket }
  ...
  async expirePendingOrders() {
    const state = this.read()
    for (const order of state.orders) if (order.status === 'PENDING') order.expiresAt = new Date(this.now() - 1).toISOString()
